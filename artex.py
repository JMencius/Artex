...
@click.option("-c", "--config", type = str, help = "Basecalling configuration, predcited by LongBow, options: [R9G2, R9G4, R9G6]")
@click.option("-w", "--work", type = str, default = "SAME AS OUTPUT PATH", help = "Workding directory, for storing intermediate results")
...
@click.version_option(version="0.1.0", prog_name = r"Artex, Artic pipeline extension for calling low coverage variants, based on Python 3.7+")
